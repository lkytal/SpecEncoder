...
def tomgf(sp, y):
    ...
    # all the peaks beyond M1+ 3da are set to 0
    y[min(math.ceil((sp["mass"] + 3) * sp["charge"] / precision), len(y)):] = 0
...
parser.add_argument(
    "--output", type=str, help="output file path", default="prediction.mgf"
)
parser.add_argument(
    "--decoy", type=str, help="output decoy file path", default="decoy_prediction.mgf"
)
...
decoy = [sp.copy() for sp in inputs]

for sp in decoy:
    sp["pep"] = todecoy(sp["pep"])
    sp["title"] = sp["pep"]
...
def predict(fn, sps):
    ...
    while len(sps) > 0:
        if len(sps) >= loop_size:
            sliced_spectra = sps[:loop_size]
            sps = sps[loop_size:]
        else:
            sliced_spectra = sps
            sps = []
...
predict(args.output, inputs)
predict(args.decoy, decoy)
