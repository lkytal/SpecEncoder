...
parser.add_argument(
    "--mgf", type=str, help="Training MGF file path", default="train.mgf"
)
parser.add_argument(
    "--output", type=str, help="output model file path", default="encoder.h5"
)
...
def AA_pairs(seq, out=None):
    if out is None:
        out = np.zeros(400, dtype="float32")
    ...
    **kws,
    ...
    for i, ks in enumerate(kernel):
        ...
        for r in range(1, tcn):
            ...
            x = Activation(act)(x)
            x = lset.ConvLayer(
                layers, kernel_size=kernel, padding="same", dilation_rate=2**r, **kws
            )(x)
            x = lset.normalizer()(x)
...
def res(
    ...
    **kws,
    ...
    if pooling == "up":
        x = lset.UpSamplingLayer(pool)(x)

    xc = conv(x, l, ks, c2d=c2d, lset=lset, norm=norm, act=act, strides=strides, **kws)

    if add:
        xc = merge(x, xc, c2d=c2d, lset=lset, norm=norm, mact=None, strides=strides)

    x = Activation(act)(xc)  # final activation, xc to x naming

    if pooling == 1 or pooling == "max":
        x = lset.MaxPoolingLayer(pool)(x)
    elif pooling == 2 or pooling == "ave":
        x = lset.AvePoolingLayer(pool)(x)
...
class data_processor:
    ...
    def process(self, sps, training=1):
        ...
        rst = config(
            {
                ...
                "pep_mapping": np.zeros((batch_size, 1), dtype="int64"),
            }
        )
        ...
        for i, sp in enumerate(sps):
            ...
            rst.pep_mapping[i] = pep_mapping[str(c) + pep]
...
class spec_encoder:
    ...
    def sp_net(hyper, act="relu", norm="in"):
        ...
        infos = k.layers.Reshape((l_size, 1))(
            k.layers.Dense(l_size, activation="sigmoid")(infos)
        )
    ...
    def auxiliary_tasks(v1, hyper, act="relu", norm="in"):
        ...
        aux_outputs.append(
            vec_dense(v1, hyper.outlen, act="softmax", normal=norm, name="length")
        )
        aux_outputs.append(vec_dense(v1, 1, normal=norm, name="rk"))
        aux_outputs.append(
            vec_dense(v1, hyper.maxc, normal=norm, act="softmax", name="charge")
        )
    ...
    @staticmethod
    def build(hyper, pep_mapping, act="relu", norm="in"):
        ...
        final_pep = k.layers.Conv1D(oh_dim, kernel_size=1, padding="same", use_bias=1)(
            sp_vector
        )
        final_pep = k.layers.Activation("softmax", name="peps", dtype="float32")(
            final_pep
        )

        fvec = Dense(256, activation="linear")(k.layers.Flatten()(sp_vector))
        fvec = K.l2_normalize(fvec, 1)

        cos_layer = Dense(
            len(pep_mapping),
            use_bias=0,
            kernel_constraint=k.constraints.unit_norm(axis=0),
            name="pep_mapping",
        )
        am_cosine = cos_layer(fvec)

        encoder = k.models.Model(inputs=model_inputs, outputs=fvec, name="encoder")
        full_model = k.models.Model(
            inputs=model_inputs,
            outputs=aux_outputs + [final_pep, am_cosine],
            name="full_model",
        )
...
class model_builder:
    class loss_fn:
        ...
        @staticmethod
        def am_softmax_loss(y_true, y_pred, scale=32, margin=0.3):
            y_pred = (y_true * (y_pred - margin) + (1 - y_true) * y_pred) * scale
            return K.categorical_crossentropy(y_true, y_pred, from_logits=True)

        @staticmethod
        def sparse_amsoftmax_loss(y_true, y_pred, scale=32, margin=0.35):
    ...
    def __init__(self, hyper, model=spec_encoder):
        self.hyper = hyper
        self.param = config()
    ...
    def set_loss(self):
        ...
        self.losses = {
            "pep_mapping": loss_fn.sparse_amsoftmax_loss,
    ...
    def build(self, pep_mapping, summary=True):
        self.dm, self.encoder = self.model.build(self.hyper, pep_mapping)

        if summary:
            self.encoder.summary()
        return self.dm, self.encoder
...
class train_mgr:
    ...
    def setup(self, **kws):
        ...
        self.callbacks = [
            ModelCheckpoint(
                "novo.hdf5", save_best_only=True, monitor="val_peps_mask_acc"
            )
        ]

        self.builder = model_builder(hyper)
        self.dm, self.encoder = self.builder.build(**kws)
        return self.encoder

    def prepare_data(self, MAX_REP=10):
        # sps = i2l(filter_spectra(readmgf(args.mgf, "hcd")))
        sps = i2l(readmgf(args.mgf, "hcd"))
        print(f"Number of spectra read:", len(sps))
        ...
        for sp in sps:
            ...
            if sp_count[spkey] < 2:
                continue  # only peptide have replicates used for training

            if not spkey in valid_added:
                sps_valid.append(sp)
                valid_added[spkey] = 1
            else:
                sps_train.append(sp)
        ...
        self.pep_mapping = {p: i for i, p in enumerate(list(valid_added.keys()))}
        print(f"Number of peptides:", len(self.pep_mapping))
    ...
    def run(self):
        ...
        self.dm.fit(
            self.data_generator(self.trainingset),
            epochs=hyper.dynamic.eps,
...
pep_mapping = manager.pep_mapping

encoder = manager.setup(pep_mapping=manager.pep_mapping, summary=1)
